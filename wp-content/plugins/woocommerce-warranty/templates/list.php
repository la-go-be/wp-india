...
SQL;
    ...
    function column_rma($item) {
        ...
        $order      = WC_Warranty_Compatibility::wc_get_order( $item['order_id'] );
        $link       = '';

        if ( $order && $order->customer_user ) {
            $link = get_edit_user_link( $order->customer_user );
        }
    ...
    function display() {
        ...
        foreach ( $this->items as $request ) {
            ...
            $refunded       = get_post_meta( $request['ID'], '_refund_amount', true );
