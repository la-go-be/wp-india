...
class Warranty_Active_Reports_List_Table extends WP_List_Table {
    ...
    function column_validity($item) {
        ...
        $warranty       = woocommerce_get_order_item_meta( $item_key, '_item_warranty', true );
        $warranty       = maybe_unserialize($warranty);
        $addon_index    = woocommerce_get_order_item_meta( $item_key, '_item_warranty_selected', true );

        if ( $warranty ) {
            if ( $warranty['type'] == 'addon_warranty' ) {

                // order's date of completion must be within the warranty period
                $completed      = get_post_meta( $order_id, '_completed_date', true);
    ...
    function column_date($item) {
        $order_id   = get_post_meta( $item->ID, '_order_id', true );
        $order      = WC_Warranty_Compatibility::wc_get_order( $order_id );
        return date_i18n( get_option('date_format') .' '. get_option('time_format'), strtotime($order->modified_date) );
    }
