...
class Warranty_Frontend {
    ...
    function show_request_button( $order ) {
        ...
        if ( in_array($order_status, $include) && Warranty_Order::order_has_warranty($order) ) {
            ...
            $permalink = esc_url( add_query_arg( 'order', $order->id, get_permalink( $page_id ) ) );
    ...
    function add_variation_data( $data, $product, $variation ) {
        $warranty   = warranty_get_product_warranty( $variation->variation_id );
        ...
        if ( isset($_REQUEST['req']) ) {
            ...
            if ( $request == 'new_warranty' ) {
                ...
                if ( $order_id && $idxs ) {
                    ...
                    if ( in_array( WC_Warranty_Compatibility::get_order_status( $order ), $include) ) {
                        $products = array();
