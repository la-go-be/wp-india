...
function warranty_get_order_item_warranty( $item ) {
    ...
    if ( version_compare(WC()->version, '2.0', '>=') ) {
        foreach ( $meta as $key => $value ) {
            if ( $key == '_item_warranty' ) {
                $warranty = array_merge( $warranty, maybe_unserialize( stripslashes($value[0]) ) );
            } elseif ( $key == '_item_warranty_selected' ) {
                $warranty['warranty_idx'] = $value[0];
            }
        }
    } else {
        foreach ( $meta as $meta ) {
            if ( $meta['meta_name'] == '_item_warranty' ) {
                ...
                $warranty['warranty_idx'] = $meta['meta_value'];
            }
        }
    }
...
function warranty_get_warranty_duration_string( $warranty, $order ) {
    ...
    if ( $warranty['type'] == 'no_warranty' ) {
        ...
        if ( $warranty['length'] == 'lifetime' ) {
            ...
            $order_date         = ($completed) ? $completed : $order->order_date;
...
function warranty_get_product_warranty( $product_id ) {
    $product = WC_Warranty_Compatibility::wc_get_product( $product_id );

    if ( $product && $product->is_type( 'variation' ) ) {
        if ( 'parent' == get_post_meta( $product->id, '_warranty_control', true ) ) {
...
function warranty_load( $request_id ) {
    ...
    if ( $warranty ) {
        ...
        if ( empty($warranty['first_name']) || empty($warranty['last_name']) ) {
            $order = WC_Warranty_Compatibility::wc_get_order( $warranty['order_id'] );

            if ( $order ) {
                $warranty['first_name'] = $order->billing_first_name;
                $warranty['last_name']  = $order->billing_last_name;
                $warranty['email']      = $order->billing_email;
            }
        }
...
function warranty_send_emails( $request_id, $status, $prev_status = '' ) {
    ...
    $order          = WC_Warranty_Compatibility::wc_get_order( get_post_meta( $request_id, '_order_id', true ) );
...
function warranty_return_product_stock( $request_id ) {
    ...
    $product        = WC_Warranty_Compatibility::wc_get_product( $request['product_id'] );
    ...
    if ( $manage_stock == 'yes' ) {
        $product->increase_stock( $request['qty'] );
    }
}
...
function warranty_refund_item( $request_id, $amount = null ) {
    ...
    if ( WC_Warranty_Compatibility::is_wc_version_gte_2_2() ) {
        ...
        if ( isset( $payment_gateways[ $order->payment_method ] ) && $payment_gateways[ $order->payment_method ]->supports( 'refunds' ) ) {
            $api_refunded = true;
            $result = $payment_gateways[ $order->payment_method ]->process_refund( $order_id, $amount, $refund_reason );
...
function warranty_variable_replacements( $input, $request_id ) {
    ...
    $order      = WC_Warranty_Compatibility::wc_get_order( $request['order_id'] );
    $product    = WC_Warranty_Compatibility::wc_get_product( $request['product_id'] );
    $store_url      = home_url();
    ...
    $reps = array(
        $order_number, $request['code'], $request['tracking_code'], $request['product_id'], get_the_title($request['product_id']),
