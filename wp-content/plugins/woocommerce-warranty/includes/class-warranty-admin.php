...
class Warranty_Admin {
    ...
    function admin_scripts() {
        ...
        if ( isset($_GET['page']) && in_array( $_GET['page'], $pages ) && !wp_script_is( 'select2', 'registered' ) ) {
        ...
        if ( isset($_GET['page']) && in_array( $_GET['page'], $pages ) ) {
            ...
            wp_enqueue_style( 'select2', plugins_url( 'assets/css/select2.css', WooCommerce_Warranty::$plugin_file ) );


            if ( WC_Warranty_Compatibility::is_wc_version_gt('2.1') ) {
            ...
            if ( !wp_script_is('wc-enhanced-select', 'enqueued') ) {
                wp_enqueue_script( 'wc-enhanced-select', plugins_url( 'assets/js/wc-enhanced-select.js', WooCommerce_Warranty::$plugin_file ), array( 'jquery', 'select2' ) );
    ...
    function panel_add_custom_box() {
        ...
        $product = WC_Warranty_Compatibility::wc_get_product( $post->ID );
    ...
    public function redirect_order_to_rma_form( $order ) {
        $url = admin_url('admin.php?page=warranties-new&search_key=order_id&search_term='. $order->id);
        wp_redirect( $url );
        exit;
    }
    ...
    public function order_inline_edit_actions( $order ) {
        if ( !warranty_order_has_warranty_requests( $order->id ) ) {
            return;
        }
    ?>
    ...
    public function maybe_render_addon_options( $item_id, $item, $product ) {
        ...
        $warranty       = warranty_get_product_warranty( $product->id );
