 ...
 * Time: 오후 3:51
 */
class TreepayOTT_Gateway extends WC_Payment_Gateway
{
    ...
    public function __construct()
    {
        try {
            ...
            $this->method_title = __('TreePay OTT', 'woo-treepay-ott');
            ...
            $this->formtype = "emb"; // $this->get_option('treepay_payment_formtype');
            ...
            $this->return_URL = add_query_arg('wc-api', 'TreepayOTT_Gateway', $this->home_URL);
            $this->cancel_URL = add_query_arg('wc-api', 'TreepayOTT_Gateway', add_query_arg('cancel_ec_trans', 'true', $this->home_URL));
            ...
            add_action('woocommerce_api_treepayott_gateway', array($this, 'treepay_relay_response'));


            add_action('wp_enqueue_scripts', array($this, 'treepay_scripts'));

        } catch (Exception $ex) {
            wc_add_notice('<strong>' . __('Payment error', 'woo-treepay-ott') . '</strong>: ' . $ex->getMessage(), 'error');
            return;
        }
    }
    ...
    {
        return $this->form_fields = treepayott_setting_field();
    }
    ...
    public function admin_options()
    {
        ?>
        <h3>Treepay OTT</h3>
        <p>Treepay OTT works by adding credit card fields on the checkout and then sending the details to Treepay for
            verification.</p>
        ...
        <table class="form-table">
            ...
            if (!in_array(get_woocommerce_currency(), array('THB', 'USD', 'CAD'))) {
                ...
                <div class="inline error"><p>
                        <strong><?php _e('Gateway Disabled', 'woo-treepay-ott'); ?></strong>: <?php _e('Treepay OTT does not support your store currency.', 'woo-treepay-ott'); ?>
                    </p></div>
                <?php
                return;
            } else {
    ...
    public function payment_fields()
    {
        ...
        if ($this->formtype == "mod") {
            ...
            // 이 시점에 order 가 생성되지 않았기 때문에 Order_ID 는  있을 수 없음.
            // Order ID 대신에, Product_id + SKU 를 사용한다.
            $items = WC()->cart->get_cart();

            $this->cert_url = add_query_arg('wc-api', 'TreepayOTT_Gateway', $this->home_URL);// '';// '_cert_url(empty)';
            $this->pay_type = "PACA";
    ...
    public function process_payment($order_id)
    {
        ...
        try {
                ...
                try {
                    ...
                    if ($result) {
                        if ( $result['res_cd'] == '0000') {
                            ...
                            $order->payment_complete($result['tno']);
                            $order->add_order_note("Payment with Treepay OTT successful");
            ...
            wc_add_notice(__('Error:', 'woo-treepay-ott') . ' "' . $e->getMessage() . '"', 'error');
    ...
    public function treepay_include_tax_or_discount($length_error, $order)
    {
        try {
            ...
            if (sizeof($order->get_items()) > 0) {
                ...
                $items_string = sprintf(__('Order %s', 'woo-treepay-ott'), $order->get_order_number()) . " - " . implode(', ', $item_names);
...
//        }
    ...
    public function treepay_receipt_page($order_id)
    {
        ...
        echo '<p>' . __('Thank you for your order, please click the button below to pay with Treepay OTT.', 'woo-treepay-ott') . '</p>';
    ...
    public function generate_treepay_form($order_id)
    {
        ...
        $params['pay_type']         = 'PACA'; // PACA(3D 인증 / 원클릭) or PARC(자동결제)
        ...
        $params['cert_url']         = add_query_arg('wc-api', 'TreepayOTT_Gateway', $this->home_URL);// '';// '_cert_url(empty)';
        ...
        return '<form name="' . 'treepay_payment_form' . '">
					...
					<input class="button" id="submit_treepay_payment_form" value="' . __('Pay via Treepay OTT', 'woo-treepay-ott') . '" onclick="jsf__pay_v3d(document.treepay_payment_form)" /> 
				</form>';
    ...
    public function treepay_relay_response()
    {
        ...
        if (!isset($_REQUEST['order_id'])) {        // order_id 가 없으면 'checkout' 으로 보냄
            if ($silent_post === false)
                wp_redirect(home_url('/') . 'checkout');
            exit;
        }

        if (!isset($_REQUEST['res_cd'])) {          // res_cd 가 없으면 '/' 로 보냄
            if ($silent_post === false)
        ...
        if ($_REQUEST['res_cd'] == "0000") {
            // 결제 성공
            $this->treepay_success_handler($order, $order_id, $silent_post, $_REQUEST);
    ...
    public function treepay_success_handler($order, $order_id, $silent_post, $response)
    {

        $order->add_order_note(sprintf(__('Treepay payment completed (Order: %s). Transaction number/ID: %s.', 'woo-treepay-ott'), $order_id, $response['tno']));

        // $result = $this->treepay_inquiry_transaction($order, $order_id);

        if ($response['res_cd'] == '0000') {
            $order->payment_complete($response['tno']);
            WC()->cart->empty_cart();
            $order->add_order_note(sprintf(__('Payment completed for the  (Order: %s)', 'woo-treepay-ott'), $order->get_order_number()));
            $this->treepay_log_write('Payment completed for the   ', '(Order: ' . $order_id . ')');
            if ($silent_post === false) {
    ...
    public function treepay_error_handler($order, $order_id, $silent_post)
    {
        ...
        wc_clear_notices();
        wc_add_notice(__('Error:', 'woo-treepay-ott') . ' "' . urldecode($_REQUEST['res_msg']) . '"', 'error');

        $order->add_order_note(sprintf(__('Treepay payment failed (Order: %s). ERROR CODE: %s.', 'woo-treepay-ott'), $order->get_order_number(), $_REQUEST['res_cd'] . $_REQUEST['res_msg']));

        if ($silent_post === false)
            $this->treepay_redirect_to($order->get_checkout_payment_url(true));
    }
    ...
    public function treepay_decline_handler($order, $order_id, $silent_post)
    {
        ...
        $order->update_status('failed', __('Payment failed via TreepayOTT because of.', 'woo-treepay-ott') . '&nbsp;' . $_REQUEST['res_msg']);
    ...
    private function _processOttApprove($order, $post)
    {
        ...
        $postBody = array(
            ...
            "ret_url"          => add_query_arg('order_id', $order_id, $this->return_URL)
        );
        ...
        if (curl_getinfo($ch, CURLINFO_HTTP_CODE) == "200") {
            ...
            if ($response['res_cd'] == "0000") {
                // SUCCESS!!!
                $order->add_order_note(sprintf(__('TreepayOTT payment completed (Order: %s). Transaction number/ID: %s.', 'woo-treepay-ott'), $order->get_order_number(), $response['tno']));
                $this->treepay_log_write('pg-method', 'response  SUCCESS!!!');
                // return $response['tno'];
                return $response;

            } else if ($response['res_cd'] == "C475") {
                // Need 3d Auth
                $this->treepay_log_write('pg-method', 'response  [' . $response['res_cd'] . ":" . $response['res_msg'] . ':' . $response['auth_url'] . "]");
                $order->add_order_note($response['res_cd'] . ":" . $response['res_msg']);
                $this->treepay_log_write('pg-method', '3D auth need!!!');
//                  return $response['tno'];
                return $response;
            } else {
                // FAIL!!!
                $order->add_order_note(sprintf(__('TreepayOTT payment failed (Order: %s). ERROR CODE: %s.', 'woo-treepay-ott'), $order->get_order_number(), $response['res_cd']));
