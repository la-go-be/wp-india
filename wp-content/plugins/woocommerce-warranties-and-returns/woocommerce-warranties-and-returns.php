<?php
/*
Plugin Name: Warranties and Returns for WooCommerce | Shared By Themes24x7.com
	...
	class WC_Warranties_And_Returns {
		...
		function __construct() {
			...
			if ( is_admin() ) {
				...
				add_action( 'woocommerce_product_write_panel_tabs', array( $this, 'wc_add_product_tab' ) );
				add_action( 'woocommerce_product_write_panels', array( $this, 'wc_product_tab' ) );
			...
			add_action( 'woocommerce_after_cart_table', array( &$this, 'war_cart_help' ) );
		...
		function init_plugin() {
			...
			load_plugin_textdomain( 'wcwar', false, $dir . 'plugins' );

		}

		function wc_add_product_tab() {
...
/*			if ( $posts = $columns['description'] ){ unset( $columns['description'] ); }*/
		...
		function war_product_warranty_output() {
			...
			if ( $product->product_type == 'variable' && !empty( $product->children ) ) {
				$ids = $product->children;
...
*/
		...
		function war_order( $order ) {
			...
			<?php
				$curr_complete = get_post_meta( $order->id, '_ordered_date', true );
				...
				$curr_complete = get_post_meta( $order->id, '_completed_date', true );
				...
				?>
					<tr>
						...
						<td class="wcwar_myaccount_warranty">
							<p class="wcwar_description green">
				<?php
					$curr_args = array(
					...
					if ( !empty( $curr_req ) ) {
						...
						foreach ( $curr_req as $req ) {
							...
							$curr_terms = reset( get_the_terms( $req->ID, 'wcwar_warranty' ) );

							$return = get_post_meta( $req->ID, '_wcwar_warranty_return_request', true );
							if ( $return !== '' && $return == 'return' ) {
								if ( !empty ( $curr_terms) && $curr_terms->slug == 'completed' ) {
									...
									$curr_link[] = '<a href="' . get_permalink( $req->ID ) . '" class="wcwar_request_button button">' . __( 'View Request', 'wcwar' ) . '</a>';
								}
								else {
								...
								if ( !empty ( $curr_terms) && $curr_terms->slug == 'completed' ) {
									...
									$curr_link[] = '<a href="' . get_permalink( $req->ID ) . '" class="wcwar_request_button button">' . __( 'View Request', 'wcwar' ) . $curr_numid . '</a>';
								}
								else if( !empty ( $curr_terms) && $curr_terms->slug == 'rejected' ) {
...
/*						if ( $requested === false && $curr_multi_req == 'yes' ) {
	...
	<?php
		...
		function war_ajax_status() {
			...
			$curr_selected_term = reset( get_the_terms( $request_id, 'wcwar_warranty' ) );
			$curr_selected = ( empty( $curr_selected_term) ? 'new' : $curr_selected_term->slug );
		...
		<?php
			...
			$curr_items = $curr_order->get_items();
				...
				$curr_complete = get_post_meta( $curr_order->ID, '_completed_date', true );
				...
				if ( $has_error === false ) {
					...
					if ( count( $curr_ordered ) > 1 ) {
						...
						if ( !empty( $curr_req ) ) {
							...
							foreach( $curr_req as $curr_reqq ) {
								...
								if ( $check_return == 'return' ) {
									$return = 'return';
									$return_id = $m;
									$curr_terms = reset( get_the_terms( $curr_req[$m]->ID, 'wcwar_warranty' ) );
								}
								$m++;
							}
						}
						...
						<?php
							...
							$curr_terms = reset( get_the_terms( $curr_req_reverse[0]->ID, 'wcwar_warranty' ) );
...
/*						$curr_multi_req = get_option( 'wcwar_enable_multi_requests', 'no' );
				...
				?>
					<div class="wcwar_warranty woocommerce">
						...
						<form action="<?php the_permalink(); ?>" method="POST">
							<p class="order-info"><?php printf( __( 'Order was placed on <em>%s</em> and was completed on <em>%s</em>. To request a warranty for <em>%s</em> please fill in the form bellow.', 'wcwar'), date_i18n( get_option( 'date_format' ), strtotime( $order->order_date ) ), date_i18n( get_option( 'date_format' ), strtotime( $order->completed_date ) ), $item['name'] ); ?></p>
					<?php
		...
		function war_pending_requests() {
			...
			$cpt_count = wp_count_posts( 'wcwar_warranty_req' );
	...
	<?php
		...
		function war_cart_price( $val, $attr, $content = null ) {
			...
			if ( isset( $attr['wcwar_pa_price'] ) ) {
				$product = new WC_Product( ( isset( $attr['variation_id'] ) && $attr['variation_id'] !== 0 ? $attr['variation_id'] : $attr['product_id'] ) );

				$price = $product->get_price_html();

				return $price . '<small> + ' . wc_price( $attr['wcwar_pa_price'] ) . '*</small>';
			}
		...
		function war_cart_help() {
			printf( '<p><small>* %1$s</small></p>', __( 'Your purchase has included paid warranties!', 'wcwar' ) );
		}
