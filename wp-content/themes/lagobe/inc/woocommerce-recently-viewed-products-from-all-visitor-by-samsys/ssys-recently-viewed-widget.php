 ...
 */
function register_ssys_WC_recently_viewed_widget_new() {
    register_widget( 'Ssys_WC_Recently_Viewed_new' );
}
add_action( 'widgets_init', 'register_ssys_WC_recently_viewed_widget_new' );
 ...
 * @since   1.0.0
 */
class Ssys_WC_Recently_Viewed_new extends WP_Widget {
	...
	function __construct() {
		...
		parent::__construct(
			'Ssys_WC_Recently_Viewed_new', // Base ID
			__('WooCommerce Recently Viewed Products from all visitors by Samsys' , 'samsys-WC-recently-viewed'), // Name
			array( 'description' => __( 'Adds a widget with the recently viewed products from all website visitors. ' , 'Ssys_WC_Recently_Viewed_new' ), ) // Args
		);
	}
	...
	public function form( $instance ) {
		if ( isset( $instance[ 'title' ] ) ) {
			...
			$title = __( 'New title', 'Ssys_WC_Recently_Viewed_new' );
		}
		...
		if ( isset( $instance[ 'num_prod' ] ) ) {
			...
			$num_prod = __( '4', 'Ssys_WC_Recently_Viewed_new' );
		}
