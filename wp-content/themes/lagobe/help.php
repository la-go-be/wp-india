...
$entries = get_post_meta( get_the_ID(), 'ser_test_help_group', true );
...
<?php foreach ( (array) $entries as $key => $entry ) { ?>
	...
	<li>
		<a target="_blank" href="<?php echo $entry['product_ser_url']; ?>">
		
		<?php if ( isset( $entry['product_ser_icon'] ) ) { ?>
		<img src="<?php echo $entry['product_ser_icon']; ?>" alt="<?php echo $title = esc_html( $entry['product_ser_title'] ); ?>">
		
		<?php } ?>
		
		<?php if ( isset( $entry['product_ser_title'] ) ) { ?>
			<p><?php echo $title = esc_html( $entry['product_ser_title'] ); ?></p>
		<?php } ?>
