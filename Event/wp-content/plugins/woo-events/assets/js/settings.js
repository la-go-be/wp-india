jQuery(document).ready(function($) {
	...
	var toShow = jQuery( '#wooevents .subsubsub li:first-child a.tab' ).attr( 'href' );
	jQuery( '#wooevents .subsubsub li:first-child a.tab' ).addClass( 'current' );
	// Remove the first occurance of # from the selected string (will be added manually below).
	toShow = toShow.replace( '#', '');

	jQuery( '#wooevents h2, #wooevents form > p:not(".submit"), #wooevents table' ).hide();
	jQuery( 'h2#' + toShow ).show().nextUntil( 'h2.section-heading', 'p, table, table p' ).show();
    // Create nav links for settings page
    jQuery( '#wooevents .subsubsub a.tab' ).click( function ( e ) {
