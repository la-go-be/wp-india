 ...
 * @return string
 */
function plan_up_theme_font_url() {
	$font_url = '';

	$roboto = _x( 'on', 'Roboto font: on or off', 'plan-up' );
	$condensed = _x( 'on', 'Roboto Condensed font: on or off', 'plan-up' );
	$droid = _x( 'on', 'Droid Serif font: on or off', 'plan-up' );
	...
	if (  'off' !== $roboto || 'off' !== $condensed ) {
		...
		if ( 'off' !== $droid ) {
			$font_families[] = 'Droid Serif:700,400';
		}
		$query_args = array(
					...
					<div class="contributor-summary">
						...
						<a class="button contributor-posts-link"
						   href="<?php echo esc_url( get_author_posts_url( $contributor_id ) ); ?>">
							<?php printf( _n( '%d Article', '%d Articles', $post_count, 'plan-up' ), $post_count ); ?>
						</a>
					</div>
					<!-- .contributor-summary -->
				</div>
	 ...
	 */ {
		function fw_theme_paging_nav( $wp_query = null ) {
			...
			$links = paginate_links( array(
				...
				'prev_text' => esc_html__( '&larr; Previous', 'plan-up' ),
				'next_text' => esc_html__( 'Next &rarr;', 'plan-up' ),
			) );
			...
			if ( $links ) :
				...
				<nav class="navigation paging-navigation">
					<h1 class="screen-reader-text"><?php _e( 'Posts navigation', 'plan-up' ); ?></h1>
	 ...
	 */ {
		function fw_theme_post_nav() {
			...
			<nav class="navigation post-navigation">
				<h1 class="screen-reader-text"><?php _e( 'Post navigation', 'plan-up' ); ?></h1>

				<div class="nav-links">
					<?php
					if ( is_attachment() ) :
						previous_post_link( '%link',
							__( '<span class="meta-nav">Published In</span>%title', 'plan-up' ) );
					else :
						previous_post_link( '%link',
							__( '<span class="meta-nav">Previous Post</span>%title', 'plan-up' ) );
						next_post_link( '%link', __( '<span class="meta-nav">Next Post</span>%title', 'plan-up' ) );
					endif;
					?>
				</div>
				<!-- .nav-links -->
	 ...
	 */ {
		function fw_theme_posted_on() {
			if ( is_sticky() && is_home() && ! is_paged() ) {
				echo '<span class="featured-post">' . __( 'Sticky', 'plan-up' ) . '</span>';
			}
